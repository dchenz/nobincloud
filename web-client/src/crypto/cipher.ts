...
export async function encrypt(plain: ArrayBuffer, key: ArrayBuffer): Promise<ArrayBuffer> {
  ...
  return concatArrayBuffer(iv, await window.crypto.subtle.encrypt(cipher, _key, plain));
}
 ...
 */
export async function decrypt(enc: ArrayBuffer, key: ArrayBuffer): Promise<ArrayBuffer | null> {
