...
test("Concat array buffer", () => {
  ...
  expect(concatToString(buf1))
    .toBe(text1);

  expect(concatToString(buf3))
    .toBe(text3);

  // Two ArrayBuffer

  expect(concatToString(buf1, buf2))
    .toBe(text1 + text2);

  expect(concatToString(buf2, buf1))
    .toBe(text2 + text1);

  // ArrayBuffer with itself

  expect(concatToString(buf1, buf1))
    .toBe(text1 + text1);

  // More than two ArrayBuffer

  expect(concatToString(buf1, buf2, buf2, buf1))
    .toBe(text1 + text2 + text2 + text1);
