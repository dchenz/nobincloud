...
test("Derive main account key and upload hash to server", async () => {
  ...
  */
  ...
  expect(passwordKey.byteLength)
    .toBe(32);
  expect(Buffer.from(passwordKey).toString())
    .toBe(expectedPasswordKey.toString());
  // Hash of password key uploaded to server
  const hashedPasswordKey = await deriveServerPasswordHash(PASSWORD, passwordKey);
  expect(hashedPasswordKey.byteLength)
    .toBe(64);
